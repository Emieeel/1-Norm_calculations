...
def kernel(casci, mo_coeff=None, ci0=None, verbose=logger.NOTE, eri=None):
    '''CASCI solver
    '''
    if mo_coeff is None: mo_coeff = casci.mo_coeff
    ...
    if eri is not None:
        eri_cas = eri
        # print("ITS WORKING")
        # print("SHAPE IS", eri_cas.shape, "mo coeff.shape", mo_coeff.shape)
    else:
        # print("ITS NOT WORKING")
        eri_cas = casci.get_h2eff(mo_coeff)
        # print("SHAPE WE WANT IS", eri_cas.shape, "mo coeff.shape", mo_coeff.shape)
...
# don't call self._scf.get_veff because _scf might be DFT object
    ...
    def kernel(self, mo_coeff=None, ci0=None, verbose=None, eri=None):
        '''
        Returns:
        ...
        self.e_tot, self.e_cas, self.ci = \
                kernel(self, mo_coeff, ci0=ci0, verbose=log, eri=eri)
